...
- **The read side already has the inverse machinery.** `root-io` already models `TKey`/`TStreamerInfo`/`TBasket`/`TBranch` and already handles ROOT's compression framing. A writer reuses those type models and the compression layer; the new work is serialization-direction code, not the whole format. So we **fork/extend `root-io`** (MPL-2.0, file-level copyleft — contributing back is fine) rather than start fresh.
